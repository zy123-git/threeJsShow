...
void main() {
  ...
  //分段处理粒子动画
  //爆炸
  float explosionProgress = remap(progress, 0.0, 0.2, 0.2, 1.0);
  ...
  //放大缩小
  float scaleOpeningProgress = remap(progress, 0.0, 0.25, 0.5, 1.0);
  float scaleClosingProgress = remap(progress, 0.25, 1.0, 1.0, 0.0);
  float scaleProgress = min(scaleOpeningProgress, scaleClosingProgress);
  scaleProgress = clamp(scaleProgress, 0.0, 1.0);
  //闪烁
  float twinkProgress = remap(progress, 0.0, 0.9, 0.0, 1.0);
